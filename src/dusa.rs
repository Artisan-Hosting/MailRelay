...
use dusa_collection_utils::{errors::{ErrorArray, ErrorArrayItem, WarningArray}, stringy::Stringy, types::PathType};
use dusa_common::{check_version, prefix::{receive_message, send_message}, MessageType, RequestPayload, RequestRecsPlainText, SOCKET_PATH};
...
pub async fn encrypt_text(data: String) -> Result<Stringy, ErrorArrayItem> {
    ...
    let socket_path: PathType =  match SOCKET_PATH(false, throw_away_errors.clone(), throw_away_warning.clone()).uf_unwrap() {
        Ok(d) => {
            d.warning.display();
            d.data
        },
    ...
    if let Err(mut err) = send_message(&mut stream, &dusa_message, throw_away_errors.clone()).uf_unwrap() {
        return Err(err.pop())
    }

    thread::sleep(Duration::from_millis(100));

    let response: dusa_common::prefix::GeneralMessage = match receive_message(&mut stream, throw_away_errors.clone()).uf_unwrap() {
        Ok(d) => d,
    ...
    let data: Stringy = response.payload
    .get("value")
    .and_then(|v| v.as_str())
